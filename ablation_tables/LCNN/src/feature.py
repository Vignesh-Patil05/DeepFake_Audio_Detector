...
def calc_stft_file(datafl):
    '''
    the input is datafl.
    '''
    data = []
    labels = []
    with open(datafl) as f:
        lnum = sum(1 for _ in f)

    with open(datafl) as f:
        for line in tqdm(f, total=lnum):
            aup, lbl, lblname = line.strip().split(',')
            stft_spec = _calc_stft(aup)
            data.append(stft_spec)
            if int(lbl) == 0:
                labels.append(0)
            else:
...
def build_stft_map_from_file(datafl):
    '''
    the input is datafl.
    '''
    with open(datafl) as f:
    ...
    with open(datafl) as f:
        for line in tqdm(f, total=lnum):
            audp, lbl, lblname = line.strip().split(',')
            stft_spec = _calc_stft(audp)
            audp_feat_m[audp] = stft_spec
...
def calc_cqt_file(datafl):
    ...
    labels = []
    with open(datafl) as f:
        for i, line in tqdm(enumerate(f), total=lnum):
            aup, lbl, lblname = line.strip().split(',')
            if int(lbl) == 0:
                labels.append(0)
            else:
            ...
            cqt_spec = _calc_cqt(aup)

            height = cqt_spec.shape[0]
            if i == 0:
                resized_data = np.zeros((lnum, height, max_width))
    ...
    return resized_data[..., np.newaxis], np.array(labels)

def load_cqt_from_file(datafl):
    with open(datafl) as f:
        lnum = sum(1 for _ in f)
...
def build_cqt_map_from_file(datafl):
    with open(datafl) as f:
        lnum = sum(1 for _ in f)
