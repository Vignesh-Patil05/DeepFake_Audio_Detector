...
for i, tsne_dict in enumerate(detector_name_list):
    ...
    # try:
    scatter = tsne_draw(feat_transformed, g_label_spe, ax=axs[i], epoch=0, log='share_in_specific', detector_name='xception')
    #     vi+=1
    # except:
    #     pass
...
handles = [plt.Line2D([0], [0], marker='*', color='w', markerfacecolor=color_map[i], markersize=10) if i == 0 else plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=color_map[i], markersize=10) for i in range(len(FAKE_AUDIO_VOCODER))]
labels = [label_dict[i] for i in range(len(FAKE_AUDIO_VOCODER))]
fig.legend(handles, labels, title="Classes", loc="upper right", fontsize=14)

plt.tight_layout()
savefl = f'{CKPT_EST_FD}/tsne.png'
plt.savefig(savefl)
print(savefl)