...
class UCFDetector(AbstractDetector):
    ...
    def __init__(self, config):
        ...
        # del _ckpt
        from .config_xlsr import xlsr_cfg
        ...
        from networks.wav2vec2 import Wav2Vec2Config
        wv_cfg_b = Wav2Vec2Config()
        # import pdb;pdb.set_trace()
        # print(xlsr_cfg['model']['encoder_embed_dim'])
        for k,v in wv_cfg_b.__dict__.items():
            if k not in xlsr_cfg['model']:
                xlsr_cfg['model'][k] = v

        xlsr_cfg = Dict2Class(xlsr_cfg['model'])
        config['backbone_config'] = xlsr_cfg
    ...
    def features(self, data_dict: dict, inference=False) -> torch.tensor:
        ...
        # encoder
        feat = self.encoder.features(cat_data, padding_mask=None, mask=False, layer=None, inference = inference)
        feat = feat['features']
        feat_dict = {'feat': feat}
        return feat_dict
    ...
    def forward(self, data_dict: dict, inference=False) -> dict:
        ...
        if inference:
            ...
            pred_dict = {'cls': out.detach(), 'feat': None}
            return  pred_dict
