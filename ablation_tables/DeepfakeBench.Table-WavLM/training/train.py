...
def prepare_training_data(config):
    ...
    elif 'dataset_type' in config and config['dataset_type'] == 'pair-audio':
...
def main():
    ...
    for k,v in model.named_modules():
        if 'encoder' in k and (isinstance(v, torch.nn.BatchNorm2d) or isinstance(v,torch.nn.SyncBatchNorm)):
            v.requires_grad=False
            v.eval()

    for k,v in model.named_parameters():
        print(f"{k} {v.shape} and requires_grad: {v.requires_grad}")
