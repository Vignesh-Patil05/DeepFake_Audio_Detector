...
from dataset.test_dataset import testDataset, testDatasetAudio
...
#     test_data_loaders = {'test': test_data_loader}
#     return test_data_loaders


def prepare_testing_data(config):
    test_set = testDatasetAudio(config, mode='test')
    test_data_loader = \
        torch.utils.data.DataLoader(
            dataset=test_set, 
            batch_size=config['test_batchSize'],
...
def test_one_dataset(model, data_loader, tsne_dict):
    for i, data_dict in tqdm(enumerate(data_loader)):
        # get data
        data, label, label_spe = \
        data_dict['audio'], data_dict['label'], data_dict['label_spe']
    
        # move data to GPU
        data_dict['audio'], data_dict['label'], data_dict['label_spe'] = data.to(device), label.to(device), label_spe.to(device)
        ...
        tsne_dict['feat_content'].append(predictions['feat_content'].cpu().numpy())
        tsne_dict['feat_specific'].append(predictions['feat_specific'].cpu().numpy())
...
def test_epoch(model, test_data_loaders):
    ...
    tsne_dict['feat_content'] = np.concatenate(tsne_dict['feat_content'], axis=0)
    tsne_dict['feat_specific'] = np.concatenate(tsne_dict['feat_specific'], axis=0)
    print('===> Test Done!')
...
def main():
    ...
    # tsne_fd = osp.join(config['log_dir'], 'tsne')
    # os.makedirs(tsne_fd, exist_ok=True)
    # tsne_fl = os.path.join(tsne_fd, f"tsne_dict_{config['model_name']}_{epoch}.pkl")
    tsne_fl = config['tsne_fl']
    if tsne_fl != 'xxx':
        # save tsne
        with open(tsne_fl, 'wb') as f:
