...
def fenchel_dual_loss(l, m, measure=None):
    ...
    '''
    N, units, n_locals = l.size() # batch, number_units, ots | the number units are the number of feature maps.
    n_multis = m.size(2) # 1!

    # First we make the input tensors the right shape.
    l = l.view(N, units, n_locals)
    l = l.permute(0, 2, 1)
    l = l.reshape(-1, units) # Nxn_locals, number_units

    m = m.view(N, units, n_multis)
    m = m.permute(0, 2, 1)
    m = m.reshape(-1, units) # Nxn_multis, number_units

    # Outer product, we want a N x N x n_local x n_multi tensor.
    u = torch.mm(m, l.t()) # N x n-multis x N x n_locals
    u = u.reshape(N, n_multis, N, n_locals).permute(0, 2, 3, 1) # => N x N x n_local x n_multi

    # Since we have a big tensor with both positive and negative samples, we need to mask.
    mask = torch.eye(N).to(l.device) # [1 0] [0 1]
    n_mask = 1 - mask # [0 1] [1 0]
